$(document).ready(function() {
      ...
      $(document).ready(function() {
            ...
            // function to load all professionals
            database.ref('/Professionals').limitToFirst(4).on("child_added",
              function(snapshot) {
                // Store data in variable sv
                sv = snapshot.val();
                name = sv.name;
                ...
                var pImg = $("<img>").attr("src", imgUrl).addClass(
                  "professional-media");
                var pTitle = $("<p>").addClass("profile text-center").append(
                  title);
                var pBio = $("<p>").addClass("profile").append(bio);
                var pVideo = $("<iframe>").attr("src", videoUrl).attr(
                  "frameborder", 0).attr('allowFullScreen', '').addClass(
                  "professional-media card-video");

                var cardTitle = $("<div>").addClass("card-content").append(
                  pName, pTitle);
                var cardImg = $("<div>").addClass("card-image").append(pImg);
                var cardBody = $("<div>").addClass(
                  "card-content card-body-text").append(pBio);
                var cardVideo = $("<div>").addClass("card-image").append(
                  pVideo);
                var boxDiv = $("<div>").addClass("card").append(cardTitle,
                  cardImg, cardBody, cardVideo);
                var colDiv = $("<div>").addClass("col-sm-6 col-md-3").append(
                  boxDiv);
            ...
            // function to load specific professional
            database.ref('/Professionals').orderByChild("identifier").equalTo(
              'P001').on("child_added", function(snapshot) {
              // Store data in variables
              sv = snapshot.val();
              name = sv.name;
              title = sv.title;
            ...
            database.ref('/Careers').orderByChild("identifier").equalTo('C001')
              .on("child_added", function(snapshot) {
                // Store data in variables
                sv = snapshot.val();
                title = sv.title;
